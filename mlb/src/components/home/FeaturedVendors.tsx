      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {vendors.slice(0, 6).map((vendor, index) => (
            <motion.div key={vendor.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
              <Link href={`/vendors/${vendor.slug}`} className="group block">
                <div className="bg-white rounded-2xl overflow-hidden border border-champagne-100/50 shadow-sm premium-card">
                  <div className="relative h-56 bg-gradient-to-br from-champagne-100 to-champagne-200 overflow-hidden">
                    {vendor.coverPhoto ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <span className="text-6xl font-serif font-bold text-champagne-300">{vendor.businessName.charAt(0)}</span>
                      </div>
                    )}
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-serif font-bold text-charcoal-900 group-hover:text-champagne-700 transition-colors mb-2">{vendor.businessName}</h3>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mt-12">
          <Link href="/vendors"
            ...
            {locale === "mk" ? "Прикажи ги сите" : "View All"}
