...
export default function VendorProfileContent({ vendor }: { vendor: Vendor }) {
  ...
  return (
    <div className="min-h-screen bg-ivory-50">
      {/* Cover */}
      <div className="relative h-64 sm:h-80 lg:h-96 bg-gradient-to-br from-champagne-200 to-champagne-300">
        {vendor.coverPhoto ? (
          <img src={vendor.coverPhoto} alt={vendor.businessName} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <span className="text-8xl font-serif font-bold text-champagne-400/50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-lg border border-champagne-100 p-8 mb-6"
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  {vendor.featured && (
                    <span className="inline-flex items-center gap-1 px-3 py-1 bg-champagne-100 text-champagne-700 text-xs font-semibold rounded-full mb-3">
              ...
              {eventTypes.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-6">
                  {eventTypes.map((type) => (
                    <span key={type} className="px-3 py-1.5 bg-ivory-100 text-charcoal-600 text-sm rounded-lg border border-champagne-100">
                      {type}
                    </span>
                  ))}
                </div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="text-sm font-semibold text-champagne-700 hover:text-champagne-800"
                >
              ...
              {showReview && (
                <div className="mb-8 p-6 bg-ivory-50 rounded-xl border border-champagne-100">
              ...
              {vendor.reviews.length === 0 ? (
                ...
                <div className="space-y-6">
                  {vendor.reviews.map((review) => (
                    <div key={review.id} className="border-b border-champagne-100 pb-6 last:border-0 last:pb-0">
                              ...
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  ...
                                  className={`w-3.5 h-3.5 ${
                                    i < review.rating ? "text-champagne-500 fill-champagne-500" : "text-charcoal-200"
                                  }`}
                                />
                              ))}
                            </div>
          ...
          <div className="lg:col-span-1">
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-lg border border-champagne-100 p-6 mb-6 sticky top-28"
            >
              {!showBooking ? (
                <div className="text-center">
                  <Calendar className="w-12 h-12 text-champagne-500 mx-auto mb-4" />
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-gradient-to-r from-champagne-600 to-champagne-700 text-white font-semibold rounded-xl hover:from-champagne-700 hover:to-champagne-800 transition-all shadow-md"
                  >
                    {t.vendors.bookConsultation}
                  </button>
                </div>
              ...
              {/* Contact Info */}
              <div className="mt-6 pt-6 border-t border-champagne-100 space-y-3">
                <a href={`tel:${vendor.phone}`} className="flex items-center gap-3 text-sm text-charcoal-600 hover:text-champagne-700 transition-colors">
                  <Phone className="w-4 h-4 text-champagne-500" />
                  {vendor.phone}
                </a>
                <a href={`mailto:${vendor.email}`} className="flex items-center gap-3 text-sm text-charcoal-600 hover:text-champagne-700 transition-colors">
                  <Mail className="w-4 h-4 text-champagne-500" />
                  {vendor.email}
                </a>
                {vendor.website && (
                  <a href={vendor.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-charcoal-600 hover:text-champagne-700 transition-colors">
                    <Globe className="w-4 h-4 text-champagne-500" />
                ...
                <div className="flex gap-2 pt-2">
                  {vendor.facebook && (
                    <a href={vendor.facebook} target="_blank" rel="noopener noreferrer" className="w-9 h-9 bg-champagne-50 hover:bg-champagne-100 rounded-lg flex items-center justify-center transition-colors" aria-label="Facebook">
                      <svg className="w-4 h-4 text-champagne-700" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                    </a>
                  )}
                  {vendor.instagram && (
                    <a href={vendor.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 bg-champagne-50 hover:bg-champagne-100 rounded-lg flex items-center justify-center transition-colors" aria-label="Instagram">
                      <svg className="w-4 h-4 text-champagne-700" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" /></svg>
                    </a>
                  )}
                </div>
              </div>
